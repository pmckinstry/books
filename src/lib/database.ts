...
function initializeDatabase() {
  ...
  if (count.count === 0) {
    ...
    const popularBooks: Array<[string, string, string, string, number, string, string, string, string, string]> = [
      ...
      ['Treasure Island', 'Robert Louis Stevenson', 'A novel about a young boy who finds a treasure map and sets sail for an island.', '978-0141439730', 311, 'English', 'Penguin Classics', 'https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1327942880i/295.jpg', '1883-05-23', 'Adventure'],
      ['Kidnapped', 'Robert Louis Stevenson', 'A novel about a young man who is kidnapped and sold into slavery.', '978-0141439730', 288, 'English', 'Penguin Classics', 'https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1327942880i/2958.jpg', '1886-07-01', 'Adventure'],
      ...
      ['A Hero of Our Time', 'Mikhail Lermontov', 'A novel about a bored and cynical young officer in the Russian army.', '978-0140447951', 160, 'English', 'Penguin Classics', 'https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1327942880i/16639.jpg', '1840-01-01', 'Classic'],
      ['The Master and Margarita', 'Mikhail Bulgakov', 'A novel about the devil visiting Moscow and the story of Pontius Pilate.', '978-0141180144', 480, 'English', 'Penguin Classics', 'https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1327942880i/117833.jpg', '1967-01-01', 'Fantasy'],
      ...
      ['The Old Man and the Sea', 'Ernest Hemingway', 'A novel about an old fisherman\'s struggle with a giant marlin.', '978-0684801223', 128, 'English', 'Scribner', 'https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1327942880i/2165.jpg', '1952-09-01', 'Classic'],
      ['For Whom the Bell Tolls', 'Ernest Hemingway', 'A novel about an American fighting in the Spanish Civil War.', '978-0684803357', 480, 'English', 'Scribner', 'https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1327942880i/2166.jpg', '1940-10-21', 'Historical'],
      ['A Farewell to Arms', 'Ernest Hemingway', 'A novel about an American ambulance driver in Italy during World War I.', '978-0684801469', 332, 'English', 'Scribner', 'https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1327942880i/2167.jpg', '1929-09-27', 'Historical'],
      ['The Sun Also Rises', 'Ernest Hemingway', 'A novel about American expatriates in Paris and Spain after World War I.', '978-0684800714', 251, 'English', 'Scribner', 'https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1327942880i/2168.jpg', '1926-10-22', 'Classic'],
      ['The Grapes of Wrath', 'John Steinbeck', 'A novel about a family of tenant farmers during the Great Depression.', '978-0143039433', 464, 'English', 'Penguin Classics', 'https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1327942880i/18114322.jpg', '1939-04-14', 'Historical'],
      ['Of Mice and Men', 'John Steinbeck', 'A novel about two displaced migrant ranch workers during the Great Depression.', '978-0140177398', 112, 'English', 'Penguin Books', 'https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1511302084i/890.jpg', '1937-01-01', 'Classic'],
...
export const userBookAssociationOperations = {
  ...
  getReadBooksWithPagination: (userId: number, page: number = 1, limit: number = 10, sortBy: string = 'title', sortOrder: 'asc' | 'desc' = 'asc', search?: string): { books: (BookWithGenres & { user_association?: UserBookAssociation })[], total: number, totalPages: number } => {
    try {
      ...
      const validSortFields = ['title', 'author', 'updated_at', 'isbn', 'page_count', 'language'];
...
export const bookOperations = {
  ...
  getPaginated: (page: number = 1, limit: number = 10, sortBy: string = 'created_at', sortOrder: 'asc' | 'desc' = 'desc', search?: string): { books: BookWithGenres[], total: number, totalPages: number } => {
    ...
    // Validate sortBy parameter
    const validSortFields = ['title', 'author', 'created_at', 'isbn', 'page_count', 'language'];
    const validSortOrders = ['asc', 'desc'];
  ...
  checkDuplicate: (title: string, author: string, excludeId?: number): Book | null => {
    ...
    if (excludeId) {
      stmt = db.prepare('SELECT * FROM books WHERE LOWER(title) = LOWER(?) AND LOWER(author) = LOWER(?) AND id != ?');
      params = [title.trim(), author.trim(), excludeId];
    } else {
      stmt = db.prepare('SELECT * FROM books WHERE LOWER(title) = LOWER(?) AND LOWER(author) = LOWER(?)');
      params = [title.trim(), author.trim()];
    }
